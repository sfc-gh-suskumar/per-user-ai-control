...
if page == "Quota Management":
    ...
    st.caption("View and set per-user daily/monthly credit limits and notification thresholds.")
    ...
    if selected_quota:
        ...
        try:
            ...
            if not config.empty:
                ...
                col3.metric("Block Enforcement", "Enabled" if block_enabled else "Disabled")
            else:
        ...
        with st.form(f"set_limits_{quota}", clear_on_submit=True):
            ...
            if submitted:
                if not ticket_limits or not ticket_limits.strip():
                    st.warning("A ServiceNow ticket is required to proceed.")
                elif not new_daily and not new_monthly:
        ...
        with col_add:
            with st.form(f"add_threshold_{quota}", clear_on_submit=True):
                ...
                thr_cycle = st.selectbox("Cycle", ["DAILY", "MONTHLY"], key=f"thr_cycle_{quota}")
        ...
        with col_remove:
            with st.form(f"remove_threshold_{quota}", clear_on_submit=True):
                ...
                thr_cycle_rm = st.selectbox("Cycle", ["DAILY", "MONTHLY"], key=f"thr_cycle_rm_{quota}")
